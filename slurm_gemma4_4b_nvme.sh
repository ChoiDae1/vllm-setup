#!/bin/bash
#SBATCH --job-name=vllm-gemma4-4b
...
slack_notify ":rocket: *Gemma4 4B vLLM 서버 시작*
- Job ID: \`${SLURM_JOB_ID}\`
- Node: \`${NODE}\`
...
# ── 종료 트랩 (크래시 포함) ──────────────────────────────────
trap 'slack_notify ":stop_sign: *Gemma4 4B 서버 종료*  Job \`${SLURM_JOB_ID}\` | $(date +'"'"'%Y-%m-%d %H:%M:%S'"'"')"; rm -rf "${LOCAL_BASE}"' EXIT
