#!/bin/bash
#SBATCH --job-name=vllm-gemma4-31b-2
...
#SBATCH --output=/mnt/lustre/slurm/users/daewon/evospec/logs/gemma4_31b_%j.out
#SBATCH --error=/mnt/lustre/slurm/users/daewon/evospec/logs/gemma4_31b_%j.err
...
slack_notify ":rocket: *Gemma4 31B-2 vLLM 서버 시작*
- Job ID: \`${SLURM_JOB_ID}\`
- Node: \`${NODE}\`
- Endpoint: \`http://${NODE}:8003/v1\`
- 시작: $(date '+%Y-%m-%d %H:%M:%S')"

# ── 종료 트랩 (크래시 포함) ──────────────────────────────────
trap 'slack_notify ":stop_sign: *Gemma4 31B-2 서버 종료*  Job \`${SLURM_JOB_ID}\` | $(date +'"'"'%Y-%m-%d %H:%M:%S'"'"')"; rm -rf "${LOCAL_BASE}"' EXIT

# ── vLLM 서버 실행 ────────────────────────────────────────────
# --enforce-eager: CUDA graph 비활성화 → shm_broadcast hang 방지
vllm serve google/gemma-4-31B-it \
    ...
    --served-model-name gemma4-31b-2
