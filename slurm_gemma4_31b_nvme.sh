#!/bin/bash
#SBATCH --job-name=vllm-gemma4-31b
...
slack_notify ":rocket: *Gemma4 31B vLLM 서버 시작*
- Job ID: \`${SLURM_JOB_ID}\`
- Node: \`${NODE}\`
- Endpoint: \`http://${NODE}:8001/v1\`
- 시작: $(date '+%Y-%m-%d %H:%M:%S')"

# ── 종료 트랩 (크래시 포함) ──────────────────────────────────
trap 'slack_notify ":stop_sign: *Gemma4 31B 서버 종료*  Job \`${SLURM_JOB_ID}\` | $(date +'"'"'%Y-%m-%d %H:%M:%S'"'"')"; rm -rf "${LOCAL_BASE}"' EXIT
