#!/bin/bash
#SBATCH --job-name=vllm-qwen3-emb-8b
...
#SBATCH --output=/mnt/lustre/slurm/users/daewon/evospec/logs/qwen3_emb_8b_%j.out
#SBATCH --error=/mnt/lustre/slurm/users/daewon/evospec/logs/qwen3_emb_8b_%j.err
...
echo "Model      : Qwen/Qwen3-Embedding-8B"
echo "Port       : 8002"
echo "Start time : $(date)"
echo "=============================================="
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader

slack_notify ":mag: *Qwen3-Embedding-8B vLLM 서버 시작*
- Job ID: \`${SLURM_JOB_ID}\`
- Node: \`${NODE}\`
...
# ── 종료 트랩 (크래시 포함) ──────────────────────────────────
trap 'slack_notify ":stop_sign: *Qwen3-Embedding-8B 서버 종료*  Job \`${SLURM_JOB_ID}\` | $(date +'"'"'%Y-%m-%d %H:%M:%S'"'"')"; rm -rf "${LOCAL_BASE}"' EXIT
...
vllm serve Qwen/Qwen3-Embedding-8B \
    ...
    --served-model-name qwen3-emb-8b
